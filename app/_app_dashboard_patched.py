...
def _resolve_bundle_dir():
    # 1) Respetar variable de entorno si viene seteada desde la Celda 15
    ...
    try:
        ...
        candidates += [
            _repo_root / "out" / "dashboard_bundle",          # Celda 14 oficial
            _repo_root / "out",                                # por si están sueltos
    ...
    for c in candidates:
        try:
            if c and c.exists() and all((c / f).exists() for f in req):
                return c
        except Exception:
            pass
    # fallback: usa env si existe, o el cwd
